...
// WHEN DOWNLOADING FROM POLYHAVEN

// change resolution from 4k to 1k
// export ZIP files

// file contents when downloading textures
...
// Bump - like the normal map, but it's a grayscale value (usually don't need)
// Metal - defines the metallic parts (on metallic textures)
// AO/Rough/Metal will combine AO, Roughness, and Metal maps into one by saving them in different channels (red, green, blue)
...
// Normally we use webp for most textures and PNG for the Normal

// using webps to optimize performance
...
// set color space so THREE.js knows it's encoded in sRGB
floorColorTexture.colorSpace = THREE.SRGBColorSpace;

// have textures repeat
...
const graveARMTexture = textureLoader.load('./grave/plastered_stone_wall_1k/plastered_stone_wall_diff_1k.webp');
...
const roof = new THREE.Mesh(
    new THREE.ConeGeometry(3.5, 1.5, 4), // radius, height, radial segments
    new THREE.MeshStandardMaterial({
...
door.position.y = 1;
door.position.z = 2.01; // add 0.01 to prevent z-fighting
house.add(door);

// Bushes
const bushGeometry = new THREE.SphereGeometry(1, 16, 16);
const bushMaterial = new THREE.MeshStandardMaterial({
    color: '#ccffcc', // make them more green
...
// FogExp2 is more realistic, just indicate color and density
...
const tick = () => {
    ...
    ghost1.position.y = Math.sin(ghost1Angle) * Math.sin(ghost1Angle * 2.35) * Math.sin(ghost1Angle * 3.5); // make a bumpy pattern

    const ghost2Angle = - elapsedTime * 0.3;
    ghost2.position.x = Math.cos(ghost2Angle) * 5;
    ghost2.position.z = Math.sin(ghost2Angle) * 5;
    ghost2.position.y = Math.sin(ghost2Angle) * Math.sin(ghost2Angle * 2.35) * Math.sin(ghost2Angle * 3.5);

    const ghost3Angle = elapsedTime * 0.2;
    ghost3.position.x = Math.cos(ghost3Angle) * 6;
    ghost3.position.z = Math.sin(ghost3Angle) * 6;
    ghost3.position.y = Math.sin(ghost3Angle) * Math.sin(ghost3Angle * 2.35) * Math.sin(ghost3Angle * 3.5);
